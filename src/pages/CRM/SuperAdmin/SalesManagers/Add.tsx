...
export default function AddSalesManagerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axiosInstance.post("/sales/managers", payload);
      if (res.data.success) {
        toast.success(res.data.message || "Sales Manager created");
        navigate("/crm/superadmin/sales-managers");
      }
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to create sales manager");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        <Button 
          variant="ghost" 
          size="sm"
          onClick={() => navigate("/crm/superadmin/sales-managers")}
          className="gap-2"
        >
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Add Sales Manager</h1>
          <p className="text-gray-600 mt-1">Create a new sales manager account</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Fill in the details to create and invite a new sales manager.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
            ...
            <div className="flex gap-3 justify-end">
              <Button type="button" variant="outline" onClick={() => navigate("/crm/superadmin/sales-managers")} disabled={loading} className="h-11 w-full sm:w-auto">Cancel</Button>
