...
import { questionRecommendationService, CreateQuestionData } from '@/api/questionBankService';
import { toast } from 'sonner';

interface RecommendQuestionFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

const RecommendQuestionForm: React.FC<RecommendQuestionFormProps> = ({
  open,
  onOpenChange,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await questionRecommendationService.createRecommendation(formData);
      if (response.success) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Recommend Question for Question Bank</DialogTitle>
        </DialogHeader>
...
export default RecommendQuestionForm;
